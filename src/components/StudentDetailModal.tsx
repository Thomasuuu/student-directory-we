...
import { ArrowsOutSimple, User, Envelope, Phone, IdentificationCard, GraduationCap } from '@phosphor-icons/react';
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md glass-elevated border-border/50 max-h-[90vh] overflow-y-auto">
          <DialogHeader className="pb-2">
            ...
            <DialogDescription className="sr-only">
              รายละเอียดข้อมูลติดต่อและรูปภาพของ {student.fullName}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-col items-center gap-3 sm:gap-4">
              ...
              <div className="flex flex-col items-center gap-2">
                <Button
                  ...
                  className="h-9 rounded-full border-border/60 bg-background/75 px-4 backdrop-blur-sm"
                >
      ...
      <Dialog open={imageOpen} onOpenChange={setImageOpen}>
        <DialogContent className="max-w-[96vw] w-[min(96vw,760px)] overflow-hidden glass-elevated border-border/50 bg-background/95 p-0 text-foreground shadow-2xl">
          <div className="border-b border-border/60 bg-background/80 px-5 py-4 pr-14 backdrop-blur-sm">
            <DialogTitle className="text-sm font-semibold text-foreground">
              {student.fullName}
            </DialogTitle>
            <DialogDescription className="sr-only">
              รูปภาพเต็มของ {student.fullName}
            </DialogDescription>
          </div>

          <div className="flex items-center justify-center bg-muted/25 p-4 sm:p-6">
            <div className="w-full max-w-[620px] overflow-hidden rounded-2xl border border-border/60 bg-background shadow-xl">
              <img
                src={student.photoUrl}
                alt={student.fullName}
